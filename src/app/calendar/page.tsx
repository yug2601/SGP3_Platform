...
export default function CalendarPage() {
  ...
  const [qc, setQc] = useState({ title: '', projectId: '', priority: 'medium' as TaskPriority })

  // ICS import/export helpers
  const [icsProjectId, setIcsProjectId] = useState('')
  ...
  async function quickCreate() {
    ...
    const created = await api<Task>("/api/tasks", { method: 'POST', body: JSON.stringify({
    ...
    setQc({ title: '', projectId: qc.projectId, priority: qc.priority })
    setQcOpen(false)
  }
        ...
        <Card>
          <CardContent className="p-0">
            <div className="grid grid-cols-7 gap-px bg-border min-h-64">
              {days.map((d, idx) => (
                <div key={idx} onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDayDrop(e, d)} className="bg-background p-2">
                  <DayList date={d} />
                </div>
              ))}
            </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 text-sm text-muted-foreground">
                ...
                <span>{selectedTasks.length} task{selectedTasks.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <select className="px-2 py-2 rounded-md border bg-background" value={icsProjectId} onChange={(e) => setIcsProjectId(e.target.value)}>
              <option value="">ICS project…</option>
              {projects.map(p => (<option key={p.id} value={p.id}>{p.name}</option>))}
            </select>
          ...
          <CardContent className="p-4">
            <div className="grid md:grid-cols-4 gap-3 items-end">
              <div>
                ...
                <Input placeholder={`Task for ${selectedDate.toLocaleDateString()}`} value={qc.title} onChange={(e) => setQc(v => ({ ...v, title: e.target.value }))} />
              </div>
              <div>
                <label className="text-sm font-medium">Project</label>
                <select className="w-full px-3 py-2 rounded-md border bg-background" value={qc.projectId} onChange={(e) => setQc(v => ({ ...v, projectId: e.target.value }))}>
              ...
              <div>
                ...
                <select className="w-full px-3 py-2 rounded-md border bg-background" value={qc.priority} onChange={(e) => setQc(v => ({ ...v, priority: e.target.value as TaskPriority }))}>
              ...
              <div className="flex gap-2">
                <Button onClick={quickCreate} className="w-full"><Plus className="h-4 w-4 mr-2" />Create ({selectedDate.toLocaleDateString()})</Button>
                <Button variant="ghost" onClick={() => setQcOpen(false)}>Close</Button>
              </div>
            </div>
          </CardContent>
        </Card>
